...
- **Kotlin Parity Test Suite (`CorpusParityTest.kt`)**: Implemented a JUnit 4 test suite in `android/app/src/test/java/com/sacredring/android/CorpusParityTest.kt` that loads `corpus_parity.json` and evaluates Android's `ChordInterpreter` against JS web player ground truth for Roman numerals, letter names, and pitch-class sets.
