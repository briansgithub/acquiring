<#
    ...
    Updates the database only, unless -Publish is given.
      ...
      .\tooling\commands\Sync-Catalog.ps1                 # update the catalog
      .\tooling\commands\Sync-Catalog.ps1 -Publish        # ...and ship the Android asset
      .\tooling\commands\Sync-Catalog.ps1 -DryRun         # report only, zero requests
      .\tooling\commands\Sync-Catalog.ps1 -ConfirmHooktheoryAuthorization # authorized live run
      .\tooling\commands\Sync-Catalog.ps1 -WithArtistSweep    # add the slow, low-yield channel
      .\tooling\commands\Sync-Catalog.ps1 -CdxMaxAgeDays 7    # re-pull the archive index sooner
#>
[CmdletBinding()]
param(
    [switch]$Publish,
...
You can inspect the local plan safely with: .\tooling\commands\Sync-Catalog.ps1 -DryRun
"@
}
