...
Self-contained module at `tooling/_Research_testing/hooktheory_catalog/`. Discovers TheoryTab songs from Hooktheory (Meilisearch + legacy URL list + light crawl), stores metadata in SQLite, and enriches each song with Hooktheory SongMetrics, chord/transition stats, and a normalized 0–100 complexity rating. Respects API rate limits and uses conservative pacing for Puppeteer enrichment.
...
```bash
# From repo root
cd tooling/_Research_testing/hooktheory_catalog
...
like — from the repo root, `.\tooling\commands\Sync-Catalog.ps1` does the same thing.
...
```powershell
.\tooling\commands\Sync-Catalog.ps1 -ConfirmHooktheoryAuthorization
...
```powershell
.\tooling\commands\Register-SyncTask.ps1                 # daily at 04:00, database only
.\tooling\commands\Register-SyncTask.ps1 -At 02:30       # different time
.\tooling\commands\Register-SyncTask.ps1 -Unregister     # remove
...
the ~62 MB Android asset stays a deliberate
...
Web UI: start the player with `python tooling/commands/launch_player.py` (or
`node web/server.js`). The **Song Selector** panel (left column of `index.html`)
uses `/api/library`. Catalog admin page: `/catalog.html` via `/api/catalog/*`.
...
```powershell
.\tooling\commands\Start-Recovery.ps1              # resume (detached; survives closing the window)
.\tooling\commands\Start-Recovery.ps1 -Foreground  # run in this window, Ctrl+C stops
.\tooling\commands\Status-Recovery.ps1             # progress bar, phase counts, log tail
.\tooling\commands\Stop-Recovery.ps1               # halt between items; -Force to kill
```
...
**Fetch** is the only step that opens Hooktheory in a browser. Other steps read `tooling/_Decode_oracle/out/<slug>/scrape.json` locally. **metadata** / **processed** / **tested** return 409 if harvest artifact is missing.
...
- Does not replace the oracle harness (`tooling/_Decode_oracle/`) — shares API/extractor only.
