...
| `oracle_out_dir` | Oracle run | Yes — relative path to `tooling/_Decode_oracle/out/<slug>/` |
...
Re-enrichment script: `tooling/_Debug_testing/re_enrich_test_batch.cjs`

```bash
node tooling/_Debug_testing/re_enrich_test_batch.cjs --limit 20
```

Report: `tooling/_Debug_testing/hooktheory_catalog/re_enrich_report.json`

Batch = first 20 songs by `first_seen_at` (discovery order).

**2026-06-29 run:** `20/20 ok` in ~2m 13s. Report: `tooling/_Debug_testing/hooktheory_catalog/re_enrich_report.json`
