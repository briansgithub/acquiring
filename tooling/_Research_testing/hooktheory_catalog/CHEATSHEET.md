...
Run from `tooling/_Research_testing/hooktheory_catalog/` unless noted. Root shims (`node status.js`) work the same as `cli/*`.
...
| `.\tooling\commands\Sync-Catalog.ps1` (repo root) | Same, from PowerShell (`-Publish`, `-DryRun`, `-CdxMaxAgeDays`) |
...
| Command | What it does |
|---------|----------------|
| `.\tooling\commands\Register-SyncTask.ps1 -ConfirmHooktheoryAuthorization` (repo root) | After written authorization, register a daily 04:00 sync; DB only |
| `.\tooling\commands\Register-SyncTask.ps1 -At 02:30` | Same, different time |
| `.\tooling\commands\Register-SyncTask.ps1 -Unregister` | Remove the scheduled task |
...
| Command | What it does |
|---------|----------------|
| `.\tooling\commands\Start-Recovery.ps1` | Resume the full recovery run, detached |
| `.\tooling\commands\Start-Recovery.ps1 -Foreground` | Same, in this window (Ctrl+C stops) |
...
| Command | What it does |
|---------|----------------|
| `python tooling/commands/launch_player.py` | Free port 3000, start server, Ctrl+C / Quit stops |
