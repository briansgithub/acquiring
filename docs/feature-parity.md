...
| F021 | Pitch, key, scale, mode, transposition, and enharmonic engine | Complete | Partial | P0 | XL | `MusicTheory.kt`; parity corpus | Swift domain rules execute the shared corpus and focused tests; the full Android theory suite and UI consumers remain. |
| F022 | Chord parsing, interpretation, scale degrees, and voicing | Complete | Partial | P0 | XL | `ChordInterpreter.kt`; parity corpus | All 45 shared chord cases pass; Android edge-case suite and fitted consumers remain. |
| F023 | Fitted Roman-numeral renderer | Complete | Not started | P1 | XL | `RomanNumeralLayout.kt`; `RomanNumeralRenderer.kt` | Text labels are temporary; required Canvas/Core Text layout is not implemented. |
| F024 | Fitted scale-degree renderer | Complete | Not started | P1 | L | `ScaleDegreeRenderer.kt` | Required accidental/hat vector renderer is not implemented. |

## Playback and quiz

| ID | Feature | Android status | iOS status | Priority | Complexity | Android reference | iOS notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| F025 | Full timeline with melody/chords/key changes/playhead | Complete | Partial | P0 | XL | `MainActivity.kt`; `QuizPlaybackTiming.kt` | Pure chord/melody events, Canvas chord lane, and transport-driven playhead exist; visual melody lane, key regions, scrolling, and full semantics remain. |
| F026 | Root-only Simple mode and interval slider | Complete | Partial | P1 | L | `MainActivity.kt`; `RootIntervalPreview.kt`; `QuizIntervalState.kt` | Root-only audio mode exists; interval slider/cards remain. |
| F027 | App-scoped looping transport and view continuity | Complete | Partial | P0 | XL | `QuizPlaybackController.kt`; `QuizPlaybackEngine.kt` | AppEnvironment owns a source-node loop renderer; elapsed publishing and view/background continuity evidence remain. |
| F028 | Tap seek, drag/inertia scrub, pause/resume, Simple seek | Complete | Partial | P1 | XL | `MainActivity.kt`; `QuizPlaybackTiming.kt` | Slider seek and pause/resume exist; timeline tap, gesture inertia, pause-on-scrub, and parity tests remain. |
| F029 | Draggable play/pause, reset, section switch, continuation | Complete | Partial | P1 | L | `MainActivity.kt`; `QuizPlaybackController.kt` | Play/pause and section reload exist; dragging, reset, persisted position, and requested-state continuation remain. |
| F030 | Quiz tempo from 0–200 percent | Complete | Partial | P1 | M | `MainActivity.kt`; `QuizPlaybackEngine.kt` | Live 0–200% control rebuilds deterministic event timing; continuation and edge-case parity tests remain. |
| F031 | Global transposition from -12 to +12 semitones | Complete | Partial | P0 | L | `MainActivity.kt`; `AudioPitchAdapter.kt`; `QuizPlaybackEngine.kt` | UI range and pitch calculation exist; live reconfiguration/card/preview consistency tests remain. |
| F032 | Ten synthesized waveforms/instruments | Complete | Partial | P1 | XL | `SynthVoice.kt`; audio engines | Ten native mathematical voices and deterministic tests exist; listening/level/route evidence remains. |
...
| F034 | Independent melody/chord transport balance | Complete | Partial | P1 | M | `MainActivity.kt`; `QuizDial.kt`; `QuizPlaybackEngine.kt` | Independent event gains and an accessible balance slider exist; custom dial, gain smoothing, and listening tests remain. |
| F035 | Active melody, interval, chord, and chord-tone cards | Complete | Partial | P0 | XL | `MainActivity.kt`; `QuizIntervalState.kt` | Transport progress now drives an accessible current-chord/root card; active melody, interval, and chord-tone cards remain. |
| F036 | Exclusive card/interval/chord/exact-frequency previews | Complete | Partial | P0 | XL | `AudioEngine.kt`; `RootIntervalPreview.kt`; `MainActivity.kt` | Root preview and isolated preview node exist; all card variants and transport-pause policy remain. |
| F037 | Lock in Major relative-Ionian labels/rendering | Complete | Partial | P1 | L | `RelativeIonianContext.kt`; `MainActivity.kt` | Android's fixed-context degree, staff, preview-register, enharmonic, and chord-label tests are ported; the Quiz toggle changes the current theory card without changing playback, while full notation consumers remain. |
| F038 | Native PCM output, synthesis, fades, cancellation, recovery | Complete | Partial | P0 | XL | `AppAudioOutput.kt`; audio engines; `SynthVoice.kt` | Pure preview/loop renderers, envelopes, clipping, cancellation entry points, and source node exist; underrun/recovery/device evidence remains. |
...
| F043 | Two-note capture, name, cents, and playback | Complete | Partial | P1 | XL | `HummingIntervalPopup.kt`; `SpelledInterval.kt`; `ComfortablePitchCapture.kt` | Two-slot capture and measured directional shorthand/cents exist; stable-window capture and pair playback remain. |
| F044 | Target listening with spelled pitch and cents | Complete | Partial | P1 | L | `HummingIntervalPopup.kt`; `SingingTargets.kt` | Live note/cents feedback exists; target-mode transition and full spelling parity remain. |
...
| F049 | Tessitura target placement and continuity | Complete | Partial | P1 | XL | `TessituraResolver.kt`; `TessituraSessionViewModel.kt`; `MainActivity.kt` | Pure target/interval resolution and session anchor exist; contour continuity and all practice consumers remain. |
