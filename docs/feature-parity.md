...
| F046 | Persistent root/melody/chord-tone practice | Complete | Partial | P1 | XL | `PersistentQuizPitchPractice.kt`; `MainActivity.kt` | Explicit persistent pitch sheet now uses the low-latency 1,024/256 fast detector and no-lag smoothing profile; transport-following root/melody/chord-tone target kinds remain. |
| F047 | Melody marker and median run scoring | Complete | Partial | P1 | XL | `PersistentQuizPitchPractice.kt`; `MainActivity.kt` | Portable target selection, contiguous-run grouping, settle gating, median/outlier scoring, feedback thresholds, and formatting are ported with tests. The live Quiz timeline marker, transport-following sampler, score badges, and physical-device evidence are deferred until after the barebones iPhone checkpoint. |
| F048 | Three-second tessitura calibration states | Complete | Partial | P1 | XL | `TessituraControl.kt`; `ComfortablePitchCapture.kt`; `TessituraSessionViewModel.kt` | Three-second capture, final-window averaging, silence pause, dropout grace/restart, retry, cancel, progress, and deterministic tests exist; non-dismissable presentation and device evidence remain. |
