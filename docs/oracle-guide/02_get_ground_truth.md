...
Goal: produce `tooling/_Decode_oracle/out/<slug>/scrape.json` for a set of **unique, not-yet-processed** songs. That JSON is the ground truth everything else compares against.
...
`tooling/_Research_testing/discover_theorytab_search.js` walks Hooktheory search for `a–z` / `0–9` and appends new TheoryTab URLs to `tooling/_Research_testing/discovered_urls.json`:

```bash
node tooling/_Research_testing/discover_theorytab_search.js
```

There is also `tooling/_Research_testing/discover_theorytab_urls.js` for link-crawl discovery. Both are additive and idempotent (dedupe into the same file).
...
```bash
node tooling/_Decode_oracle/corpus4/buildCorpus4.js --target 500          # writes corpus4.json
node tooling/_Decode_oracle/corpus4/buildCorpus4.js --target 500 --dry-run # preview, no write
```

How it picks songs (in [`buildCorpus4.js`](../../tooling/_Decode_oracle/corpus4/buildCorpus4.js)):
...
node tooling/_Decode_oracle/batchScrapeCorpus.js --corpus tooling/_Decode_oracle/corpus4.json --limit 20   # first 20 only
node tooling/_Decode_oracle/batchScrapeCorpus.js --corpus tooling/_Decode_oracle/corpus4.json --compare     # also run compare per song
```
...
- Each song lands in `tooling/_Decode_oracle/out/<slug>/scrape.json` plus screenshots.
...
```bash
node tooling/_Decode_oracle/run.js https://www.hooktheory.com/theorytab/view/artist/song --no-browser
```
...
```bash
node tooling/_Research_testing/corpus4_status.js                              # how many of the manifest scraped OK
node tooling/_Research_testing/corpus4_backfill.js                            # replace failed entries with new pool URLs
```
