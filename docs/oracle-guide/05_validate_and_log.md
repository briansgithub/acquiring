...
3. **Cross-check earlier corpora** so you don't break past wins:
   ```bash
   node tooling/_Decode_oracle/buildChordDb.js --corpus tooling/_Decode_oracle/corpus2.json --db-dir tooling/_Decode_oracle/chord_db_corpus2
   node tooling/_Decode_oracle/buildChordDb.js --corpus tooling/_Decode_oracle/corpus3.json --db-dir tooling/_Decode_oracle/chord_db_corpus3
   ```
...
4. **Tier-1 regression songs** — must stay 100% `notesOk`:
   ...
   node tooling/_Decode_oracle/compare.js tooling/_Decode_oracle/out/the-beatles__eleanor-rigby/scrape.json
   ```
...
1. **Append a numbered entry** to [`tooling/_Decode_oracle/DECODE_FIX_LOG.md`](../../tooling/_Decode_oracle/DECODE_FIX_LOG.md), continuing the sequence (find the last `## Fix NNN`). Each entry states: **symptom → root cause → fix → files → exposed-by song(s)**, plus the before/after DB numbers. Use sub-letters (036a, 036b, …) when one session lands several related fixes.
2. **Update the status table** in [`README.md`](README.md) if any corpus-wide number changed.
3. **Record deferrals** (single-song edge cases, unimplemented features) at the end of your fix-log entry and/or in [`tooling/_Decode_oracle/REMAINING_FAILURES.md`](../../tooling/_Decode_oracle/REMAINING_FAILURES.md) so nobody re-chases them.
...
Authoritative list: [`tooling/_Decode_oracle/REMAINING_FAILURES.md`](../../tooling/_Decode_oracle/REMAINING_FAILURES.md). Categories you will keep seeing:
...
- Debug scripts/output → `tooling/_Debug_testing/`; research scripts/output → `tooling/_Research_testing/`.
