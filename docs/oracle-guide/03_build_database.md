...
```bash
node -e "const fs=require('fs');console.log(fs.readFileSync('tooling/_Decode_oracle/chord_db_corpus4/SUMMARY.md','utf8').split('\n').slice(0,40).join('\n'))"
node tooling/_Decode_oracle/testModification.js --list   --db-dir tooling/_Decode_oracle/chord_db_corpus4   # all buckets + pass rate
node tooling/_Decode_oracle/testModification.js --failing --db-dir tooling/_Decode_oracle/chord_db_corpus4   # buckets below 99%
```
