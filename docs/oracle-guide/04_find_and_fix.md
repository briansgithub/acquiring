...
`tooling/_Debug_testing/diffSignature.cjs` reads the built DB (no scraping) and prints `truthPcs` vs `engPcs` for engine failures, grouped or filtered:

```bash
# summary: count of engine failures per chord "signature"
node tooling/_Debug_testing/diffSignature.cjs

# rows matching ALL tokens (type/inv/applied/bor/sus/alt/add/omit)
node tooling/_Debug_testing/diffSignature.cjs type=7 inv=3
node tooling/_Debug_testing/diffSignature.cjs "omit=3&5" --limit 8
node tooling/_Debug_testing/diffSignature.cjs bor=locrian

# options
node tooling/_Debug_testing/diffSignature.cjs alt=b5 --all          # include harness/piano failures too
node tooling/_Debug_testing/diffSignature.cjs --db chord_db_corpus2 type=9   # different corpus DB
```
...
node tooling/_Decode_oracle/testModification.js bor=locrian --rerun --db-dir tooling/_Decode_oracle/chord_db_corpus4
...
node tooling/_Debug_testing/diffSignature.cjs    # confirm the engine-failure count dropped
```
...
1. **Diff:** `node tooling/_Debug_testing/diffSignature.cjs bor=locrian` showed `iø7(loc)` truth `[1,4,7,10]` vs eng `[1,5,7,10]` / `[…]+b7` across multiple songs — a shared pattern, not a one-off.
