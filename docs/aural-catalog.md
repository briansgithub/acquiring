...
Song title, artist, and section appear immediately. **Open in Playback** routes through the same full-song loader used by Library selection, so it includes melody and every standard Playback control, alongside the exact source section and highlighted passage. **Jump to quiz passage** returns the timeline to the matched sequence's first beat. **Return to quiz** and Android Back restore the current question and draft. Exploring Playback marks the attempt assisted. Source loading failures retain the quiz. Persisted source revision, snapshot, seed, target, response, phase, exposure, and assistance restore an interrupted Playback round-trip after process recreation. Quiz audio and microphone activity stop during navigation.
