...
3. **Complete:** Xcode created cloud-managed Apple Distribution assets and the App Store profile, build 1 was archived and validated, and the signed package was uploaded successfully. App Store Connect reported version 1.0 build 1 as Processing on September 1, 2026.
4. **Pending:** after Apple finishes processing, add the owner as an internal tester, install through TestFlight, and smoke-test launch, adaptive layout, Library empty state, catalog action entry points, basic navigation, background/foreground transitions, and a clean relaunch. Audio and microphone approval remain separate device gates.
...
- App Store Connect Terms are accepted. The private Acquiring app record and explicit App ID exist for `com.acquiring.ios`, and build 1 uploaded successfully on September 1, 2026. The current external state is Apple server-side Processing; no public submission or release has occurred.
