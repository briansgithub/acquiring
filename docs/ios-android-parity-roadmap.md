...
11. [wip] Dynamic key modulation, polyphonic audio engine, ChordInterpreter parity — bare-bones melody timeline + chord card land on Quiz (`QuizView`); fixed a real crash in it (`AudioSystem.swift`'s render closure inherited `@MainActor` isolation, illegal on the real-time audio thread). Play still doesn't reliably show "Pause": unified-log inspection during the hang shows `coreaudiod` activating *and deactivating* the simulator audio device within ~5s of the tap (fast, not stuck) — so this isn't `engine.start()` hanging as first suspected. More likely an audio interruption/route event fires right after activation, and `AppAudioSystem`'s interruption handler publishes `.paused` back through `observeTransport()` faster than XCUITest can observe the intermediate "Pause" state, or the device is being torn down entirely. Root cause still unconfirmed — deferred, tracked in `testQuizPlayTogglesToPause`. This affects every future feature that calls `environment.audio.play()`/`play(_:)` (transport and one-shot preview alike share the same engine/session), so it's worth someone revisiting with real device audio (TestFlight) rather than this host's simulator.
