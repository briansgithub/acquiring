 ...
 *
 * Deliberately NOT getRepoRoot()/android: `android/` is a nested independent
 * git repo, so it exists only in the canonical checkout — a git worktree of
 * this repo has no android/ at all, and getRepoRoot() there points at the
 * worktree. Export/publish run from a worktree therefore wrote to (and looked
 * for) a path that does not exist.
 *
 * The data root is the reliable anchor: it is configured to the canonical
 * location via SACRED_RING_DATA / sacred_ring_data.config.json, so its parent
 * is the checkout that actually owns android/.
 */
function getAndroidDir() {
  const fromEnv = process.env.SACRED_RING_ANDROID_DIR;
  if (fromEnv && String(fromEnv).trim()) return path.resolve(String(fromEnv).trim());

  const besideDataRoot = path.join(path.dirname(resolveDataRoot()), 'android');
  if (fs.existsSync(besideDataRoot)) return besideDataRoot;
