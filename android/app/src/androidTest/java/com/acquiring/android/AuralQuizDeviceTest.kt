...
class AuralQuizDeviceTest {
    ...
    @Test fun progressionPlaysToCompletionAndNextRetainsTheChosenVariant() {
        ...
        assertEquals("guided", session.view().exercise!!.skillId)
        assertFalse(session.view().heard)
        assertEquals(1, session.view().progress.attempts)
        compose.onNodeWithTag("AuralPhase-reproduce").performScrollTo().performClick()
        compose.onNodeWithTag("AuralPhase-reproduce").assertIsSelected()
