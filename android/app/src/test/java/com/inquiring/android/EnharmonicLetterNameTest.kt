...
class EnharmonicLetterNameTest {
    ...
    fun testAbMinorScaleEnharmonics() {
        val key = KeyInfo("Ab", "minor")
        // Ab minor scale degrees: 1=Ab, 2=Bb, 3=Cb, 4=Db, 5=Eb, 6=Fb, 7=Gb
        assertEquals("Ab", MusicTheory.getNoteLabel(1, key.tonic, key.scale))
        assertEquals("Bb", MusicTheory.getNoteLabel(2, key.tonic, key.scale))
        assertEquals("Cb", MusicTheory.getNoteLabel(3, key.tonic, key.scale))
        assertEquals("Db", MusicTheory.getNoteLabel(4, key.tonic, key.scale))
        assertEquals("Eb", MusicTheory.getNoteLabel(5, key.tonic, key.scale))
        assertEquals("Fb", MusicTheory.getNoteLabel(6, key.tonic, key.scale))
        assertEquals("Gb", MusicTheory.getNoteLabel(7, key.tonic, key.scale))
        ...
        assertEquals("Abm", ChordInterpreter.getLetterName(chordI, key))
        assertEquals("Cb", ChordInterpreter.getLetterName(chordIII, key))
        assertEquals("Fb", ChordInterpreter.getLetterName(chordVI, key))

        println("✅ Ab minor enharmonics verified: i=Abm, III=Cb, VI=Fb")
    }
    ...
    fun testGbMajorScaleEnharmonics() {
        val key = KeyInfo("Gb", "major")
        // Gb major scale: 1=Gb, 2=Ab, 3=Bb, 4=Cb, 5=Db, 6=Eb, 7=F
        assertEquals("Cb", MusicTheory.getNoteLabel(4, key.tonic, key.scale))

        val chordIV = buildChordJson(4) // IV = Cb
        assertEquals("Cb", ChordInterpreter.getLetterName(chordIV, key))

        println("✅ Gb major enharmonics verified: IV=Cb")
    }
    ...
    fun testSlashBassEnharmonics() {
        ...
        assertEquals("Abm/Cb", ChordInterpreter.getLetterName(chordInv1, key))

        println("✅ Slash bass enharmonic verified: Abm/Cb")
    }
}
