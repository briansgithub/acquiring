# Sacred Ring: Android to iOS Conversion Master Plan

This document provides a comprehensive, structured strategy for porting the Sacred Ring application from Android (Kotlin/Compose) to iOS (Swift/SwiftUI).

## 1. Project Overview & Scope
Sacred Ring is a music-theoretical tool and ear training app. The core value lies in its ability to:
...
The Android app uses a custom `AudioEngine` with Sawtooth/Sine waveforms. On iOS, we will use `AVAudioSourceNode` to render the same math-based waveforms to ensure the "Sacred Ring" sound is identical.
